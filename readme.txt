...
The main navigation uses the standard WordPress menu. Support for dropdown menus is inluded by default. If you'd like to envoke a button toggle for the main navigation menu on small screens, simply uncomment the two lines from the flex_scripts_styles() function within functions.php to register and enqueue the necessary javascript file, and BAM! You're done!

Templates

Flex includes a set of your most common theme templates, including templates for Full-Width pages, Front-page, EDD Store, Right Sidebar (default), Front-Page, Tag,
...
Widgets are a great way of adding extra content to your site and Flex has a whole assortment of them.
...
Use the following instructions to install & activate Flex using your preferred method.

Manual installation:

1. Unzip the files from the Flex zip file that you downloaded
2. Upload the Flex folder to your /wp-content/themes/ directory
3. Click on the Appearance > Themes menu option in the WordPress Dashboard
4. Click the Activate link below the Flex preview thumbnail
...
3. Type 'flex' in the search field, without the quotes, and then click the Search button
4. Click the Install Now link below the Flex preview thumbnail
...
Since Flex is a starter theme to kick off your own awesome theme, the first thing you want to do is copy the Flex theme folder 
...
1. Search for Flex inside single quotations to capture the text domain.
2. Search for flex_ to capture all the function names.
3. Search for Flex with a space before it to replace all the occurrences of it in comments.
...
Flex is licensed under the [GNU General Public License version 2](http://www.gnu.org/licenses/old-licenses/gpl-2.0.html).
...
Flex utilises the following awesomeness:
